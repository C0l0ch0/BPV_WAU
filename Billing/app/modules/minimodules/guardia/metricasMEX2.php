...
<?php
  ...
  function getData($name,$nombre1){
    ...
    while ($r= odbc_fetch_object($result)){
      if(@$r1=odbc_fetch_object($result1) and
      @$r2=odbc_fetch_object($result2) and 
    ...
    if($nombre1 == "SAGUA"){
        ...
        if(@$r1->$name == ''){$r1->$name=0;}
        if(@$r2->$name == ''){$r2->$name=0;}
        $dataArray = $dataArray."data5.addRow([".$r->HORA.", ".(($r2->$name+$r3->$name)/2).", ".$r->$name." , ".$r1->$name."]);\n";
      } 
    }
    }
    }
  ...
  if($r2->SAMEX<=0) {$div = $r->SAGUA;}else{$div = $r2->SAMEX;}
    $body = "<tr><td align=\"center\"> SAMEX </td><td align=\"right\">".number_format($r->SAMEX)."</td><td align=\"right\">".number_format($r2->SAMEX)."</td><td align=\"right\">  ".number_format((($r->SAMEX*100)/$div));
    $body = $body. "%</td></tr>";
  if($r2->SAPER<=0) {$div = $r->SAGUA;}else{$div = $r2->SAPER;}
    $body = $body. "<tr><td align=\"center\"> SAPER </td><td align=\"right\">".number_format($r->SAPER)."</td><td align=\"right\">".number_format($r2->SAPER)."</td><td align=\"right\">  ".number_format((($r->SAPER*100)/$div));
    $body = $body. "%</td></tr>";
  if($r2->SAECU<=0) {$div = $r->SAGUA;}else{$div = $r2->SAECU;}
    $body = $body. "<tr><td align=\"center\"> SAECU </td><td align=\"right\">".number_format($r->SAECU)."</td><td align=\"right\">".number_format($r2->SAECU)."</td><td align=\"right\">  ".number_format((($r->SAECU*100)/$div));
    $body = $body. "%</td></tr>";
    return $body;
  }
  function getDataTableSACA(){
    ...
    $result2 = odbc_exec($conexion,"GetpercentageSA");
    $r2= odbc_fetch_object($result2);
  if($r2->SANIC<=0) {$div = $r->SAGUA;}else{$div = $r2->SANIC;}
    $body = "<tr><td align=\"center\"> SANIC </td><td align=\"right\">".number_format($r->SANIC)."</td><td align=\"right\">".number_format($r2->SANIC)."</td><td align=\"right\">  ".number_format((($r->SANIC*100)/$div));
    $body = $body. "%</td></tr>";
  if($r2->SAPAN<=0) {$div = $r->SAGUA;}else{$div = $r2->SAPAN;}
    $body = $body. "<tr><td align=\"center\"> SAPAN </td><td align=\"right\">".number_format($r->SAPAN)."</td><td align=\"right\">".number_format($r2->SAPAN)."</td><td align=\"right\">  ".number_format((($r->SAPAN*100)/$div));
    $body = $body. "%</td></tr>";
  if($r2->SAGUA<=0) {$div = $r->SAGUA;}else{$div = $r2->SAGUA;}
  $body = $body. "<tr><td align=\"center\"> SAGUA </td><td align=\"right\">".number_format($r->SAGUA)."</td><td align=\"right\">".number_format($r2->SAGUA)."</td><td align=\"right\">  ".number_format((($r->SAGUA*100)/$div), 0, '.', '');
    $body = $body. "%</td></tr>";
    return $body;
  }
  function getAccess(){
  ...
  <div class="navbar">
    <div class="navbar-inner">
      <div class="container-fluid">
        ...
        <H4><a class="brand"><span STYLE="font-size: 12pt;font-weight:bold;">WAU MOVIL COBROS</span></a></H4>
          ...
          <ul class="nav pull-right">           
            ...
            <li class="dropdown">
              ...
              <ul class="dropdown-menu">
                ...
                <li><a href="http://<?=URL.DS;?>usuario/profiles"><i class="halflings-icon user"></i> Profile</a></li>
                <li><a href="http://<?=URL.DS;?>login/logout"><i class="halflings-icon off"></i> Logout</a></li>
              </ul>
            </li>
            <!-- end: User Dropdown -->
          </ul>
...
<div class="CSSTableGenerator" style="display:inline-block;">
   ...
   <div style="display:inline-block;"></div>
<div  class="CSSTableGenerator " style="display:inline-block; vertical-align: top;">
    <table >
      <tr>
        <td >Super Agregador</td>
        <td >Cobros Hoy</td>
...
<div class="box span4" onTablet="span4" onDesktop="span4" >
  ...
  <div class="box span4" onTablet="span4" onDesktop="span4">
    <div class="box-header">
      <h2><i class="halflings-icon white check"></i><span class="break"></span>SAGUA</h2>
      <div class="box-icon">
        <a href="#" class="btn-minimize"><i class="halflings-icon white chevron-up"></i></a>
      </div>
    </div>
    <center>
    <div class="box-content">
      <div id="SAGUA" style= "height: 250px;"></div>
    </div>
    </center>
  </div><!--/span-->
...
*/
    ...
    formatter2.format(data2, 3);
        ...
        data3.addColumn('number', 'Promedio');
    ...
    formatter4.format(data4, 3);
        var chart4 = new google.visualization.AreaChart(document.getElementById('SAPAN'));
        chart4.draw(data4, options4);
    /*----------------------*/
        var data5 = new google.visualization.DataTable();
        data5.addColumn('number', 'Hora');
        data5.addColumn('number', 'Promedio');
        data5.addColumn('number', 'Cobros Ayer');
        data5.addColumn('number', 'Cobros Hoy');
        <?php
          echo $data7;
        ?>
        var options5 = {
          title: '<?php echo 'Cobros '.$name7;?>',
          hAxis: {title: 'Hora',  titleTextStyle: {color: '#333'}, ticks: data4.getDistinctValues(0)},
          vAxis: {minValue: 0},
          pointSize: 5,
          series:{
        ...
        formatter5.format(data5, 2);
    formatter5.format(data5, 3);
        var chart5 = new google.visualization.AreaChart(document.getElementById('SAGUA'));
