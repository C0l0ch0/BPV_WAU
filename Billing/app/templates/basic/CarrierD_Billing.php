                        ...
                        <table class="table table-invoice">
                            ...
                            <tbody>
                                <?php 
                                	...
                                	$query = '	select it.name, sum(r.gross) GROSS
                                                from '.$schema.'.resume_billing r
                                                left join '.$schema.'.product_master pm on r.product_id = pm.product_id
                                                left join '.$schema.'.integratordetails it on pm.integror_id = it.integrator_id
                                                where   r.price is not null 
                                                        and r.created_date = date(date_add(now(), interval -6 hour))
                                                group by it.name
                                                order by sum(r.gross) desc;';
