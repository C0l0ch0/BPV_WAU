...
$query3 = ' select * from (select id_transaction,max(cantidad) as Total,fecha
            from billing.'.$name1.'_i_p
            where id_integrador = '.$integrador.' and id_producto = '.$producto.'
...
<body>
	<?php include(TEMPLATE.DS.'top.php');?>
		...
		<div id="content" class="content">			
			<!-- begin page-header -->
			<h1 class="page-header"><?php echo strtoupper($name1);?> <small>Detalle Productos Por Integrador Movistar <?php echo $pais;?></small></h1>
			<!-- end page-header -->
                        ...
                        <div class="panel-body panel-form">
                            <form class="form-horizontal form-bordered" method ='POST'>
								<div class="form-group">
									...
									<div class="col-md-8">
									    <select class="form-control selectpicker" data-size="10" data-live-search="true" data-style="btn-white" id="combo1" name="combo1"; onchange="submit();";>
                                            <?php
                                            	$plat = 1;
								...
								?>
								<div class="form-group">
									<label class="control-label col-md-4">Producto <?php echo $producto;?></label>
									<div class="col-md-8">
									    <select class="form-control selectpicker" data-size="10" data-live-search="true" data-style="btn-white" id="combo2" name="combo2"; onchange="submit();";>
                                            <?php
                                            	$plat = 1;
                                            	$horaActual = date('H', (strtotime ("+1 Hours")));
                                            	try{
                                            	$conexion = new MySQL(0);
												$query = '	select id_producto,descripcion 
															from billing.'.strtolower($name1).'_i_p
					 										where fecha = 	date(date_add((NOW()), INTERVAL -6 hour))
																			and id_integrador = '.$integrador.' 
															group by id_integrador, descripcion
															order by descripcion asc;';
												$combo= $conexion->consulta($query);
												if ((! $combo)){
    												...
    												if($producto == 0){
    													echo "<option value=0 selected>Producto</option>";
    												}
    												while ($dato= $conexion->fetch_row($combo)){
    													if($producto == $dato[0]){
                                							$tituloG = $dato[1];
								...
								<table class="table">
									...
									<tbody>
										<tr> <?php
											...
											where created_date >= left(NOW() - INTERVAL 6 HOUR,10) and type in ("DB") and status = "Success" and integrator_id ='.$integrador.' and product_id = '.$producto.' and integrator_id <> 0;';
											...
											and type in ("DB") and status = "Success" and integrator_id ='.$integrador.' and product_id = '.$producto.' and integrator_id <> 0;';
											//echo $query;
											$exec = $conexion->consulta($query);
											$row = $conexion->fetch_row($exec);
											$value2 = $row[0]/7;
                                ...
                                <table id="data-table69" class="table table-striped table-bordered">
									...
									<?php
										...
										$query = '	select a.product_id,b.product_name,sum(a.qty)
                                                    ...
                                                    where a.integrator_id = '.$integrador.' and a.product_id = '.$producto.' and a.created_date = date(date_add((NOW()), INTERVAL -6 hour))
										...
										while ($row2= $conexion->fetch_row($exec)){
    										echo "<tr><td>$row2[0]</td><td>$row2[1]</td><td>".number_format($row2[2])."</td></tr>";
    									}
    									echo"</tbody>"
									?>
                                </table>
