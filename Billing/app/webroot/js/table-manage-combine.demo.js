    ...
    if ($('#data-table').length !== 0) {
        if ($(window).width() >= 767) {
            var table = $('#data-table').DataTable({
                //ajax:           location.origin+"/app/webroot/plugins/DataTables/json/scroller-demo.json",
                dom: 'TRC<"clear">lfrtip',
                tableTools: {
                    "sSwfPath": location.origin+"/app/webroot/plugins/DataTables/swf/copy_csv_xls_pdf.swf"
                },
                "lengthMenu": [20, 40, 60]
            });
